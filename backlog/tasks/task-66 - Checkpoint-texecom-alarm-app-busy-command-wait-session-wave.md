...
- [x] #2 Coverage and lint bars still hold (pytest --cov=texecom_alarm --cov-fail-under=90; ruff check + ruff format --check)
- [x] #3 FakePanel e2e: timeout-with-events then new-request ACK keeps Connection on; silent timeout and NAK still turn Connection off immediately
<!-- AC:END -->
...
## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 Session health in docs/architecture.md matches ADR-022: busy Arm/Disarm wait retries as a new request; Connection off on refuse, silent timeout, or exhausted retries
- [x] #2 ADR-022 live-only walks (real trigger-then-Disarm flood) are not claimed by CI
- [x] #3 No skip-and-resync path was reintroduced
<!-- DOD:END -->
