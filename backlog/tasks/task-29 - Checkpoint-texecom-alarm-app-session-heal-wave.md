...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Rename + heal suites pass (cd texecom-alarm-app && python -m pytest tests/test_mqtt_discovery.py tests/test_session_heal.py tests/test_reconnect.py tests/test_panel_trust.py tests/test_e2e_fake_panel.py -q exits 0)
- [x] #2 FakePanel health-check death → keep-trying reconnect and stuck-trust → bounded re-login paths stay green
...
## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 Discovery friendly name is Alarm Panel Connection; heal paths never blank zone/alarm availability solely for panel recovery (ADR-004)
- [x] #2 ADR-011 satisfied: health-check death reconnects; stuck trust corroborates then bounded re-login; no arm/disarm auto-retry
- [x] #3 Fail window default 90s remains documented as tunable, not final
<!-- DOD:END -->
