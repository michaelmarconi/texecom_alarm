...
running and restores live monitoring on its own — without a manual add-on restart
and without blanking zone/alarm entities solely because recovery is underway.
**Alarm Panel Connection** stays off only while we cannot talk to the panel (the
session is dead or an arm/disarm was refused or timed out) and while that
dead-session recovery is still failing; it stays on through the health-check
patience window and through a first-attempt re-login after a successful command
whose later housekeeping read did not parse. After a dead session, it returns to
live with zone/alarm state re-synced from the panel. That freshness entity’s
...
- After a mid-run death (including when unanswered health checks continue past
  the patience window), keep running and keep trying until monitoring is live
  again — no operator restart required.
...
- Keep failure visible: recovery attempts and failures are clear in everyday logs
  (not only TRACE); the connection signal does not look live while a dead-session
  recovery is still failing.
...
Then the add-on keeps running and **Alarm Panel Connection** stays on through the
patience window. If unanswered checks continue past that window, Connection goes
off and the add-on keeps trying. When the panel accepts again, Connection returns
to live with zone/alarm state re-synced — without an operator restarting the
add-on.

- **How we'll know:** end-to-end test (stand-in: FakePanel that leaves check-ins
  unanswered inside the patience window then past it, then accepts again;
  recording MQTT broker)
...
Given recovery is underway (Connection off for a dead session, or still on
through patience or a first-attempt re-login after a successful command), When
Home Assistant inspects zone and alarm entities, Then they stay available with
last-known state (not marked unavailable solely because of panel recovery).
...
Given a dead-session recovery is still failing or retrying, When the household or
automations look at **Alarm Panel Connection** and the add-on logs, Then
Connection remains off until monitoring is truly live again, and recovery
attempts/failures are visible at normal log levels (not TRACE-only). Connection
does not go off for unanswered health checks still inside the patience window, or
for a first-attempt re-login after a successful arm or disarm whose later
housekeeping read did not parse.

- **How we'll know:** end-to-end test (stand-in: FakePanel that refuses recovery
  for N attempts — assert MQTT connection signal off + log lines; FakePanel that
  leaves check-ins unanswered inside patience, or that ACKs arm/disarm then
  returns unparseable housekeeping — assert Connection stays on); optional manual
...
- As Home Assistant automations, I want **Alarm Panel Connection** to stay off
  only while we cannot talk and a dead-session recovery is still failing, to stay
  on through the patience window and through a first-attempt re-login after a
  successful command, and to return to on after a dead recovery only when live
  again, so I can gate on that signal (and raise my own alerts) without treating
  a refused arm or a hung-up panel as still live.
...
- Recovery succeeds after many failures of a dead session: Connection stays off
  throughout that reconnect; only after re-sync does it return to live — no
...
- ~~How mid-run health-check timeout should join existing clean-disconnect
  ...
  a research spike. **Narrowed 2026-08-31:** An unanswered health check starts a
  patience window; Connection stays on until that window is exceeded. Then the
  same keep-trying reconnect heal as a clean disconnect.
