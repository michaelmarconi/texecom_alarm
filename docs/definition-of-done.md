...
## Build passes
Build succeeds (`pip install -e ".[dev]"` in `texecom-alarm-app/` exits 0).

## Tests pass
All automated tests pass (`pytest` in `texecom-alarm-app/` exits 0), including
...
`ruff check` and `ruff format --check` pass in `texecom-alarm-app/`; the same
