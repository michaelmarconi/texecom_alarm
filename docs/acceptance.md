...
|---|----------|--------|-------|
| 1 | Replacement is live | ✅ pass | ~40 in-use zones, alarm entity, Alarm Panel Connection ON; add-on on dedicated ComIP `192.168.1.51`; the prior MQTT bridge not installed |
...
- **I check:** MQTT zone states (~40 in-use), alarm `disarmed`, `texecom/panel_connection/state` ON, add-on `panel_host` `192.168.1.51`, no the prior MQTT bridge add-on.
...
- HA was already running; cold-start left the add-on on ComIP `192.168.1.51` (not the old signalling-module address).
