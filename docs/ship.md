...
| RISK-017 fingerprint | fail | Critical/High residuals remain: `docs/ha-alarm-usage-spec.md` (household names, zone map, automations); brief “Current setup” LAN IPs; spike/accept docs with live IPs and UDL confirmation. `TODO.md` says `/ship` stops while those remain. Consumer README/DOCS were sanitised; pipeline artefacts were not. GitHub is already public — this is residual exposure, not a future-only risk. |
