...
3. Ensure **Texecom Alarm** (`local_texecom_alarm`) is installed; if `panel_host` is empty, apply sim defaults (`192.0.2.10`, Part-Arm Night/Home/Unused, MQTT → `core-mosquitto`); start it.
...
**Expected leftovers:** Supervisor add-on metrics (`binary_sensor.texecom_alarm_running`, etc.) — not MQTT zone entities. Prefixed zones like `binary_sensor.texecom_alarm_study_example_pir_4` after a **fresh** start are normal panel inventory (`_4` = zone number), not wipe dirt.
