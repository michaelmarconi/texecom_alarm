...
- Host LAN interface: `eth0` (`192.0.2.12`).
- `the prior MQTT bridge` add-on: slug `the prior add-on`, v1.3.1, runs on the internal
  `hassio` Docker bridge (`host_network: false`, container IP `198.51.100.2`) — **not**
  host networking. Its traffic to the panel is NAT'd out through `eth0`.
- Panel: `192.0.2.10`, default port (10001), no `udl_password` currently set in the
  add-on options.
...
```bash
tcpdump -i eth0 host 192.0.2.10 -w texecom_capture.pcap
```
