...
- **Panel under test:** Elite 88 at `192.168.1.183:10001`, UDL `1234` (SPIKE-001); practitioner reports `the prior MQTT bridge` stopped so the single ComIP slot is free.
...
1. Connect to `TEXECOM_HOST`/`TEXECOM_PORT` (defaults `192.168.1.183:10001`), wait ≥500ms, `LOGIN` with `TEXECOM_UDL_PASSWORD` (default `1234`).
...
Raw output of `experiment.py` against `192.168.1.183:10001` (UDL default `1234`), 2026-08-04 15:15:13–15 UTC-adjacent local time:

```
[2026-08-04 15:15:13] SPIKE-006 GetZoneState probe
[2026-08-04 15:15:13] Target: 192.168.1.183:10001
