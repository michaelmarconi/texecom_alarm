...
`192.168.1.183:10001` (the panel address/port recorded in `docs/brief.md`) succeeded
(`cat < /dev/null > /dev/tcp/192.168.1.183/10001` returned exit 0, versus a connection-refused
...
| Panel reports per-zone name/text without config input | `GETZONEDETAILS` returns a non-empty decoded name string for in-use zones | **40/88 zone slots returned non-empty, human-readable name text** (e.g. `'FRONT DOOR'`, `'MICHAEL STDY PIR'`, `'MSTR BED PANIC'`); the other 48 all decoded as `zoneType=0` (Unused) with empty text — consistent with unprogrammed hardware zone slots, not a decode failure |
...
[ok] panel reports 88 zones
  ...
  zone  4: type=Interior               name='MICHAEL STDY PIR'
  ...
  zone 13: type=Interior               name='ETHANS RM L WIND'
  zone 14: type=Interior               name='ETHANS RM R WIND'
...
zone names matching the household's actual rooms/openings (`FRONT DOOR`, `MICHAEL STDY PIR`,
